...
truthRE <- matrix(rt(N*3,4)/sqrt(2)*sqrt(random_effect_var),nrow=N,ncol=ncol(Y))
Y <- Y + matrix(rt(length(Y),4)/sqrt(2)*sqrt(residual_var),nrow=nrow(Y),ncol=ncol(Y))
Y <- Y + truthRE[df$id,]

coef00 <- c(0,0,c(1,4,7,1)/100,0,0)
B00 <- splines2::ibs(df$ageori,knots=knot,degree=2,intercept=TRUE,Boundary.knots=c(0,120))

Y[,1] <- Y[,1] + f_sigmoid(df$ageori,2,70,4) #B00 %*% coef00
...
est$truth <- f_sigmoid(ages,2,70,4)#spline.basis %*% coef00[3:6]
est$age <- ages
